...
function PackageCard({ product }: { product: Product }) {
  ...
  return (
    <article className={cn("group relative flex h-full flex-col overflow-hidden rounded-[1.75rem] border bg-white transition duration-300 hover:-translate-y-1 hover:shadow-xl", isFeatured ? "border-green shadow-xl shadow-green/10 ring-1 ring-green/20" : "border-slate-200 shadow-sm")}>
      ...
      <Link href={`/produkter/${product.slug}`} className="relative block h-56 overflow-hidden border-b border-slate-100 bg-gradient-to-br from-slate-50 via-white to-green/5">
      ...
      <div className="grid flex-1 grid-rows-[auto_3.5rem_2.75rem_auto_1fr_auto] p-6">
        <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-green">{position.label}</p>
        <Link href={`/produkter/${product.slug}`} className="flex items-start"><h3 className="mt-2 text-xl font-bold tracking-tight text-navy transition group-hover:text-green">{cleanName(product.name)}</h3></Link>
        <p className="mt-1 text-sm font-medium leading-relaxed text-slate-600">{position.audience}</p>
        ...
        <div className="pt-6">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            <Link href={`/produkter/${product.slug}`} className="inline-flex min-h-11 items-center justify-center rounded-full border border-slate-200 px-4 py-2.5 text-sm font-semibold text-navy transition hover:border-navy hover:bg-slate-50">Se pakken</Link>
            <button type="button" onClick={() => addItem(product)} className="min-h-11 rounded-full bg-green px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-green-hover">Læg i kurv</button>
          </div>
        </div>
      </div>
    </article>
    ...
    <section id="pakker" className="scroll-mt-24 border-b border-slate-100 bg-[#f6f8f7] py-16 md:py-20">
      <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{serviceValue.map((item) => <div key={item.number} className="flex min-h-36 flex-col rounded-2xl border border-white bg-white/90 p-4 shadow-sm"><span className="text-xs font-bold text-green">{item.number}</span><p className="mt-auto pt-4 text-sm font-bold text-navy">{item.title}</p><p className="mt-1 text-xs leading-relaxed text-muted">{item.text}</p></div>)}</div>
        </div>
